...
export class SearchPageComponent implements AfterViewInit, OnInit {
	...
	private initMap(): void {
		...
		document
			...
			.setAttribute("title", "Search by address, city, or zip code");
		(
			document.querySelector(".leaflet-control-geocoder-form")!
	...
	ngOnInit(): void {
		...
		this.getUserSubscription = this.apiService
			.getUser(this.service.getUsernameFromToken())
			.subscribe({
				next: (user: User) => {
		...
		this.getMarkersSubscription = this.apiService.getMarkers().subscribe({
			next: (markers: Marker[]) => {
				...
				this.markers.forEach(marker => {
					this.mapMarkers[marker.id] = L.marker(
						...
						.on("click", () => {
							this.latLng =
								this.mapMarkers[marker.id].getLatLng();

							const modalTrigger =
								document.getElementById("jobModalTrigger");
							modalTrigger!.click();
							...
							newMarkerModal!.addEventListener(
								"hidden.bs.modal",
								() => {
									this.companyName = "";
									this.name = "";
		...
		this.getFavoritesSubscription = this.apiService
			.getFavorites(this.service.getUsernameFromToken())
			.subscribe({
		...
		if (navigator.geolocation) {
			...
			const geolocationModal =
				document.getElementById("geolocationSupport");
	...
	submitMarkerToMap(): void {
		...
		this.currentMarker = L.marker(this.latLng, {
			...
			.on("click", () => {
				...
				this.currentJob = this.jobMarkers.find(
					job => job.id === this.currentID
				)!;
				this.isOwner =
					this.currentJob.by === this.currentUser!.username;
			});
	...
	addToFavorites(): void {
		const favorite: Favorite = this.jobMarkers.find(
			job => job.id === this.currentID
		)!;
	...
	isAlreadyContacted(): boolean {
		return this.chats.some(
			chat =>
				chat.user1 === this.currentUser!.username &&
				chat.user2 === this.currentJob.by
		);
	}
