...
export class ProfilePageComponent implements OnInit {
  ...
  confirmEditPC(): void {
    const user: User = {
      ...
      password: this.currentUser!.password,
      username: this.currentUser!.username,
      companyName: this.currentUser!.position === null ? this.positionCompany : this.currentUser!.companyName,
      position: this.currentUser!.position === null ? this.currentUser!.position : this.positionCompany,
      role: this.role
    };
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.apiService.deleteUser(this.currentUser!.username).subscribe({
          next: () => {
            this.apiService.deleteMarkersByUsername(this.currentUser!.username).subscribe({
              next: () => {
