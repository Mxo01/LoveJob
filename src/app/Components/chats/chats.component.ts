...
export class ChatsComponent implements OnInit {
	...
	chatButtons: NodeListOf<HTMLElement> =
		document.querySelectorAll(".chatButton");
	...
	showChat(e: MouseEvent): void {
		...
		const targetName =
			chatButton.firstChild!.firstChild!.firstChild!.firstChild!
				.textContent;
		this.chatName = targetName === null ? "" : targetName;
		const targetUsername =
			chatButton.firstChild!.firstChild!.childNodes[1].textContent;
	...
	sendMessage() {
		...
		const chatID = this.allChats.find(
			c => c.user1 === this.receiverUser && c.user2 === this.currentUser
		)!.id;
		const sender = this.allChats.find(
			c => c.user1 === this.receiverUser && c.user2 === this.currentUser
		)!.name2;
